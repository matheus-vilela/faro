...
export function ConfiguracoesUsuariosMembros() {
  ...
  const saveMember = async () => {
    ...
    const displayStored =
      maskWhatsappBrInput(memberPhoneDigits).trim() || null
  ...
  const toggleMemberActive = async (m: CompanyMember, active: boolean) => {
    ...
    if (active && activeCount >= 3 && !m.is_active) {
      toast.error('Limite de 3 membros ativos. Desative outro membro primeiro.')
      return
    }
    setSaving(true)
    const { error } = await supabase
  ...
  return (
    <div className="space-y-8">
      <div className="space-y-1">
        <h2 className="text-lg font-semibold tracking-tight">Usuários e membros</h2>
        <p className="text-muted-foreground text-sm max-w-2xl">
          Dados do proprietário com acesso ao sistema e membros que interagem apenas pelo
          WhatsApp (sem login no Faro).
        </p>
      </div>
      ...
      <Card className="shadow-sm border-primary/15">
        <CardHeader className="space-y-1">
          ...
          <CardDescription>
            Você é o proprietário desta empresa no Faro. O WhatsApp abaixo é o principal
            para validação das mensagens recebidas (o mesmo número da linha da plataforma é
            usado por todas as empresas; identificamos você pelo seu WhatsApp).
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-5">
          <div className="grid gap-6 sm:grid-cols-2 sm:gap-8 sm:items-start">
            <div className="space-y-2">
              <Label className="text-muted-foreground">Nome do proprietário</Label>
              <div
              ...
              <p className="text-xs text-muted-foreground">
                Igual ao nome do seu perfil no Faro. Para alterar, use os dados da sua conta.
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setOwnerPhoneDigits(
                    applyWhatsappPhoneMaskChange(ownerPhoneDigits, e.target.value),
                  )
                }
                disabled={saving}
              />
              <p className="text-xs text-muted-foreground">
                Digite com DDD. Ao salvar, o número é normalizado; se você não informar o
                código do país, <strong>55</strong> (Brasil) é adicionado automaticamente.
              </p>
            </div>
          </div>
          <div className="flex flex-wrap gap-2 pt-1">
            <Button type="button" onClick={saveOwnerPhone} disabled={saving}>
              {saving ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Salvar telefone'}
            </Button>
            {ownerPhoneDigits.trim() || currentCompany?.owner_whatsapp_normalized ? (
              <Button
                type="button"
                variant="outline"
                disabled={saving}
      ...
      <Card className="shadow-sm">
        <CardHeader className="space-y-3">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div className="space-y-1">
              <CardTitle className="text-base">Membros (somente WhatsApp)</CardTitle>
              <CardDescription className="max-w-xl">
                Membros <strong>não recebem acesso</strong> ao sistema Faro (sem login). Eles
                aparecem aqui apenas para autorizar o{' '}
                <strong>número de WhatsApp</strong> que poderá enviar mensagens pela
                integração — conforme o cadastro abaixo.
              </CardDescription>
            </div>
            <Button
              size="sm"
          ...
          <div className="flex gap-3 rounded-lg border border-amber-500/30 bg-amber-500/5 px-4 py-3 text-sm text-amber-950 dark:text-amber-100 dark:border-amber-500/40 dark:bg-amber-500/10">
            ...
            <p>
              Ao adicionar um membro, informe o <strong>número do WhatsApp</strong> (com DDD).
              Esse número será validado nas mensagens recebidas; não confunda com telefone
              fixo sem WhatsApp.
            </p>
          </div>
        </CardHeader>
                ...
                {members.map((m) => (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between gap-2 sm:justify-center">
                      <span className="text-xs text-muted-foreground sm:hidden">Ativo</span>
                      <Switch
                        checked={m.is_active}
                        onCheckedChange={(checked) => toggleMemberActive(m, checked)}
                        disabled={saving}
                      />
                    </div>
      ...
      <Sheet open={memberSheetOpen} onOpenChange={setMemberSheetOpen}>
        <SheetContent className="flex flex-col gap-0 sm:max-w-md">
          <SheetHeader>
            ...
            <SheetDescription>
              Cadastre o <strong>nome</strong> e o <strong>número de WhatsApp</strong> (celular
              com WhatsApp ativo). Este membro <strong>não terá login</strong> no Faro — apenas
              este número poderá enviar mensagens autorizadas pela integração.
            </SheetDescription>
          </SheetHeader>
          <div className="grid gap-5 py-6">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setMemberPhoneDigits(
                    applyWhatsappPhoneMaskChange(memberPhoneDigits, e.target.value),
                  )
                }
              />
            </div>
          ...
          <SheetFooter className="mt-auto border-t pt-4">
            <Button className="w-full sm:w-auto" onClick={saveMember} disabled={saving}>
              {saving ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Salvar membro'}
            </Button>
          </SheetFooter>
        </SheetContent>
      </Sheet>
