...
import { CONTAS_A_RECEBER_FLUXO_CONFIG } from "@/components/fluxo/fluxoBoletosConfigs";
import { PageHeader } from "@/components/PageHeader";
import { PageShell } from "@/components/PageShell";
import {
  CONTAS_A_RECEBER_HOME_PATH,
  CONTAS_A_RECEBER_LIST_PATH,
  contasAReceberSectionFromPath,
} from "@/lib/contasAReceberPaths";
import { cn } from "@/lib/utils";
import { CalendarDays, List, Wallet, type LucideIcon } from "lucide-react";
...
export function ContasAReceber() {
  ...
  const section = contasAReceberSectionFromPath(pathname);

  if (!section) {
    return <Navigate to={CONTAS_A_RECEBER_HOME_PATH} replace />;
  }
      ...
      <div className="flex h-full flex-col gap-6 md:flex-row md:items-start">
        <aside className="h-full w-full shrink-0 border-b border-border pb-4 md:w-56 md:border-b-0 md:border-r md:pb-0 md:pr-4">
          <nav
            ...
            aria-label="Seções de contas a receber"
          >
            {CONTAS_NAV.map((item) => {
        ...
        <div className="min-w-0 flex-1">
          <FluxoBoletosPage
            config={CONTAS_A_RECEBER_FLUXO_CONFIG}
            embedded
            section={section}
          />
        </div>
