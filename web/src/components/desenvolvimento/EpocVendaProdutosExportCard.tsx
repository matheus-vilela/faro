import {
  EpocVendaProdutosInterpretResult,
  runEpocProdutoVendasInterpret,
} from "@/components/desenvolvimento/EpocVendaProdutosInterpretCard";
...
import type { ProdutoVendasInterpretPreview } from "@/lib/epocProdutoVendasInterpret";
import {
  downloadTextAsFile,
...
export function EpocVendaProdutosExportCard() {
  ...
  const interpretCsv = async (text: string, fileName: string) => {
    ...
    try {
      const result = await runEpocProdutoVendasInterpret(
        currentCompany.id,
        text,
        fileName,
      );
  ...
  const runExport = async () => {
    ...
    try {
      ...
      setLastSummary(
        `${result.total_itens} linha(s) · ${result.dias_com_dados} dia(s) · ${result.file_name}`,
      );
      ...
      <CardContent className="space-y-4">
        ...
        {preview ? (
          <div className="border-t pt-4">
            <EpocVendaProdutosInterpretResult preview={preview} />
          </div>
        ) : null}
      </CardContent>
    </Card>
