...
export async function runEpocVendaServicosInterpret(
  companyId: string,
  text: string,
  fileName: string,
): Promise<ServicoVendasInterpretPreview> {
  const { data, error } = await supabase
    .from("services")
    .select("id, code, name, is_active")
    .eq("company_id", companyId);
  if (error) throw error;
  return previewEpocVendaServicosInterpret(text, fileName, [...(data ?? [])]);
}

export function EpocVendaServicosInterpretResult({
  preview,
}: {
  preview: ServicoVendasInterpretPreview;
}) {
  return (
      ...
      {preview.ok ? (
        <>
          ...
          <p className="text-muted-foreground text-xs">
            ...
            {preview.headers[preview.columns.vlBruto] ?? "Vl.Bruto(R$)"}. Match
            por código; códigos novos seriam criados no sync real.
          </p>
        </>
      ) : null}
    </div>
