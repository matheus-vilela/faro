...
export function FiscalNfeRecebidasManualSyncCard({
  ...
  const handleSync = useCallback(async () => {
    ...
    try {
      ...
      const novos = detail ? Number(detail.novos_xml_na_fila ?? 0) : null;
      const batchId = detail && typeof detail.batch_id === "string" ? detail.batch_id : null;
      const parts = [
        "Sincronização concluída.",
        novos !== null && !Number.isNaN(novos) ? `${novos} XML(s) novos na fila.` : null,
  ...
  }, [companyId, hasFocus, versaoInput, onSynced]);
