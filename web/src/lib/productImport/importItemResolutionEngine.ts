/**
 * Motor de resolução de linha importada (XML/NF-e) antes do estoque.
 *
 * Pontos críticos de negócio:
 * - Regras aprendidas (`import_item_resolution_rules`) têm prioridade máxima quando casam.
 * - EXPLODE_BY_RECIPE exige receita `ENTRY_BREAKDOWN` ativa; receita inativa nunca entra em auto-aplicação.
 * - Limiares 0–1 derivados de `company_product_import_settings` (0–100): auto = autoMatch/100, confirm = confirmMin/100.
...
export type ResolveXmlImportLineResult = {
  ...
  /** Produto final da linha (comercial ou SKU de estoque direto). */
  target_product_id: string | null
...
export function resolveXmlImportLine(input: ResolveXmlImportLineInput): ResolveXmlImportLineResult {
  ...
  if (stockType === "RECIPE_CONTROLLED" || stockType === "COMPOSITE") {
    /** Produto marcado como composto/receita mas sem ficha de entrada ativa — nunca auto-explodir. */
    return {
      import_nature: "REVISAO_MANUAL",
      import_engine_suggestion: "REVISAO_MANUAL",
      import_confidence_0_1: conf,
...
/** Preview determinístico de quantidades na unidade de estoque (testes / UI offline). */
export function previewEntryBreakdownQuantities(params: {
  ...
  const componentQtys = ingredients.map(
    (i) => i.qtyPerBatchInStockUnit * scale * (i.lossFactor ?? 1),
  )
