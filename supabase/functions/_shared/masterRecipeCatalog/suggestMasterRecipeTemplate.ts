import { matchRecipeFichaProhibitionLabels } from "../itemClassification/namingImportSignals.ts";
import { canonicalProductName, stripDiacriticsLower } from "../productImport/canonicalName.ts";
import { MASTER_RECIPE_DEFINITIONS } from "./seedRegistry.ts";
import type { MasterRecipeDefinition, MasterRecipeSuggestion } from "./types.ts";
...
export function suggestMasterRecipeTemplate(name: string): MasterRecipeSuggestion | null {
  ...
  return {
    ...
    score: best.score,
    explanationPt,
    likelyOperationalType: "RECEITA_FICHA",
