/**
 * Exporta relatório EPOC `mod_rel_venda_servicos` → CSV (`#tblExport` + resumo).
 ...
 *
 * Fase 2 (acoes): modulo + NaoMenu + action=FORM + token + data_de/data_ate + filtrar=FORM
 */
...
const LOG = "[epoc-export-venda-servicos]";
...
Deno.serve(async (req) => {
    ...
    return json(
      {
        ok: false,
        error:
          "Módulo de venda de serviços não carregou (sem ConteudoTela). Verifique NaoMenu/credenciais.",
      },
      502,
    );
  }

  const dayExtracts: VendaServicosDayExtract[] = [];
  for (let i = 0; i < diasConsulta.length; i++) {
    ...
    // Corpo alinhado ao curl do portal: action=FORM + filtrar=FORM (sem busca_grupo_evento).
    ...
    dayExtracts.push(
      extractVendaServicosRowsFromAcoesHtml(acoesDia.text, dia),
    );
  }

  const built = buildVendaServicosConsolidatedCsv(dayExtracts);
  ...
  const fileName = `venda-servicos-${fileStamp}.csv`;
  const storagePath =
    `${companyId}/epoc-venda-servicos/${new Date().toISOString().replace(/[:.]/g, "-")}-${fileName}`;
