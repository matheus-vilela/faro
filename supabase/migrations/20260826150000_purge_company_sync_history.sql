-- Admin Faro: para sincronizações PDV/fiscal em curso, apaga o histórico
-- e conclui os cards de onboarding da unidade (não remove notas, XMLs, vendas nem despesas).
...
begin
  ...
  -- Fiscal: histórico de consultas
  delete from public.nfe_consulta_history
  where company_id = p_company_id;
  get diagnostics v_nfe_history = row_count;

  -- Fiscal: fila em curso (e jobs já terminados desta unidade)
...
revoke all on function public.purge_company_sync_history(uuid) from public;
grant execute on function public.purge_company_sync_history(uuid) to authenticated;
