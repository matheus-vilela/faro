-- CMV por produto: flag "compõe CMV" em vez de subcategoria no cadastro.
-- A folha de despesa CMV na venda vem do padrão da empresa (primeira folha CMV, priorizando nome "Outras").

ALTER TABLE public.products
  ADD COLUMN IF NOT EXISTS composes_cmv BOOLEAN NOT NULL DEFAULT true;

COMMENT ON COLUMN public.products.composes_cmv IS
  'Se true, vendas geram boleto de CMV na folha de despesa CMV da empresa; se false, não gera CMV.';
...
COMMENT ON FUNCTION public.create_revenue_entry(jsonb) IS
  'Insere receita; taxa/imposto vem de company_revenue_category_tax_settings (ou zero). CMV de venda usa composes_cmv do produto e folha CMV da empresa.';
...
COMMENT ON FUNCTION public.update_revenue_entry(jsonb) IS
  'Atualiza receita; taxa/imposto vem de company_revenue_category_tax_settings (ou zero). CMV de venda usa composes_cmv do produto e folha CMV da empresa.';
