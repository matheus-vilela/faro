-- Unificação de produtos duplicados: nomes fundidos para importação + RPC de merge.

ALTER TABLE public.products
  ADD COLUMN IF NOT EXISTS merged_catalog_names TEXT[] NOT NULL DEFAULT '{}';

COMMENT ON COLUMN public.products.merged_catalog_names IS
  'Nomes (e aliases canônicos) de produtos unificados neste cadastro; usados na importação automática para reconhecer o mesmo item.';
...
BEGIN
  ...
  -- Nomes do produto removido → aliases de importação
...
COMMENT ON FUNCTION public.merge_company_products(UUID, UUID, UUID) IS
  'Unifica p_loser em p_winner: estoque, movimentações, vínculos NF e aliases para importação futura.';

GRANT EXECUTE ON FUNCTION public.merge_company_products(UUID, UUID, UUID) TO authenticated;

-- Compatibilidade com RPC legada do onboarding
...
DECLARE
  v_result JSONB;
BEGIN
  v_result := public.merge_company_products(p_company_id, p_winner_id, p_loser_id);
  IF COALESCE((v_result->>'ok')::boolean, false) IS NOT TRUE THEN
    RAISE EXCEPTION '%', COALESCE(v_result->>'message', v_result->>'error', 'merge_failed');
  END IF;
END;
$$;

COMMENT ON FUNCTION public.merge_onboarding_products(UUID, UUID, UUID) IS
  'Wrapper de merge_company_products (legado onboarding).';
