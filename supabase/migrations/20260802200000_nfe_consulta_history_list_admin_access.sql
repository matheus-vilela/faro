-- Histórico fiscal: platform admin e membros podem listar; SECURITY DEFINER evita RLS opaca.
...
comment on function public.focus_nfe_consulta_history_list(uuid, int) is
  'Integrações Fiscal: histórico NF-e (membro ou platform admin).';
...
-- Garante política de select para admin (caso a tabela tenha sido criada depois do backfill).
drop policy if exists "platform_admins_all" on public.nfe_consulta_history;
create policy "platform_admins_all"
  on public.nfe_consulta_history
  for all
  to authenticated
