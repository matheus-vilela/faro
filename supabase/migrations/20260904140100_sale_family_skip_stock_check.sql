-- Venda de família de cardápio não exige saldo (a baixa é das variantes).
...
DECLARE
  ...
  END;
  v_is_sale_family BOOLEAN := false;
...
BEGIN
  ...
  IF v_company_id IS NULL OR NOT public.user_has_company_access(v_uid, v_company_id) THEN
  ...
  IF v_entry_mode = 'product_sale' THEN
    ...
    SELECT p.company_id, p.current_quantity, p.stock_control_type = 'SALE_FAMILY'
    INTO v_prod_company, v_stock, v_is_sale_family
    ...
    END IF;
    IF v_csv_import_job_id IS NULL AND NOT coalesce(v_is_sale_family, false) AND v_stock < v_stock_qty THEN
...
BEGIN
  ...
  IF NOT public.user_has_company_access(v_uid, v_old.company_id) THEN
    RAISE EXCEPTION 'Sem permissao para esta empresa';
  END IF;
  ...
  IF v_entry_mode = 'product_sale' THEN
    ...
    IF v_prod_company <> v_company_id THEN
      RAISE EXCEPTION 'Produto nao pertence a empresa';
    END IF;
    IF NOT coalesce(v_is_sale_family, false) AND v_stock < v_stock_qty THEN
      RAISE EXCEPTION 'Estoque insuficiente para a quantidade informada';
    END IF;
