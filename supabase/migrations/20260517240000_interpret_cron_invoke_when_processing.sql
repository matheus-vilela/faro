-- Permite o cron invocar interpret-staging mesmo com job em `processing`.
-- A Edge finaliza órfãos (sem staging / offset completo) ou retoma em `claim_vazio`.
...
comment on function public.cron_invoke_focus_get_sync_nfe_interpret() is
  'pg_cron: POST para Edge interpret staging (Vault: focus_interpret_cron_*). Recuperação de jobs órfãos na Edge.';
