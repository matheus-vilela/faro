-- Taxa/imposto na receita operacional: segundo boleto (deducao DRE) como contas a pagar no fluxo de caixa.
-- Corrige linhas ja gravadas e reaplica RPCs para projetos que ja rodaram migracoes anteriores.
--
-- O UPDATE abaixo altera flow_type e dispara o trigger: aplicar a funcao ANTES do UPDATE.
...
SET flow_type = 'payable'
WHERE b.revenue_entry_id IS NOT NULL
  AND b.flow_type = 'receivable'
  AND (
    b.description ILIKE '%Taxas/deducoes%'
    OR b.description ILIKE '%Taxas/deduções%'
  )
  AND EXISTS (
    SELECT 1 FROM public.company_categories cc
    WHERE cc.id = b.company_category_id
      AND cc.papel_receita_dre = 'DEDUCAO'
  );
...
COMMENT ON FUNCTION public.update_revenue_entry(jsonb) IS
  'Atualiza receita, recria boletos a receber e ajusta estoque em venda de produto.';

GRANT EXECUTE ON FUNCTION public.update_revenue_entry(jsonb) TO authenticated;
