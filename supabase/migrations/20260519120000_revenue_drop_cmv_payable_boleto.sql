-- Remove contas a pagar automaticas de CMV em vendas (product_sale / recipe_sale).
-- Mantem boleto payable de imposto/deducao quando configurado na categoria de receita.

-- Limpa pendencias geradas automaticamente por vendas anteriores (nao altera boletos ja pagos).
DELETE FROM public.boletos b
WHERE b.flow_type = 'payable'
  AND b.revenue_entry_id IS NOT NULL
  AND b.status = 'pending'
  AND (
    b.description = 'CMV - Venda produtos'
    OR b.description = 'Despesa: CMV - Venda produtos'
    OR b.description ILIKE 'CMV - %'
  );
...
COMMENT ON FUNCTION public.create_revenue_entry(jsonb) IS
  'Insere receita, boletos a receber e payable de imposto/deducao (DRE), baixa de estoque em venda. Nao gera boleto payable de CMV. Opcional: integration_import_batch_id. Com service_role: _csv_import_job_id para import CSV EPOC (sem exigir estoque previo em product_sale).';
