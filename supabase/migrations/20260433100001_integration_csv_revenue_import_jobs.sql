-- Fila para processar CSV de integracao (EPOC) em receitas; disparo tipico: Database Webhook no INSERT.
...
COMMENT ON TABLE public.integration_csv_revenue_import_jobs IS
  'Job enfileirado apos export CSV (ex. EPOC); Database Webhook pode invocar a edge process-integration-csv-revenue-job.';
...
COMMENT ON FUNCTION public.create_revenue_entry(jsonb) IS
  'Insere receita, boletos (DRE) e baixa de estoque em venda. Opcional: integration_import_batch_id. Com service_role: _csv_import_job_id (uuid de integration_csv_revenue_import_jobs PENDING/PROCESSING) para atuar como requested_by do job.';
