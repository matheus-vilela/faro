-- Correlação: carregar mais (limit/offset + totals).

DROP FUNCTION IF EXISTS public.dashboard_product_recipe_match_lists(UUID);
...
BEGIN
  ...
  purchase_counted AS (
    SELECT count(*)::int AS total FROM purchase_cand
  ),
  purchase_page AS (
    SELECT *
    FROM purchase_cand
  ...
  INTO v_purchases_total, v_purchases
...
COMMENT ON FUNCTION public.dashboard_product_recipe_match_lists(UUID, INT, INT, INT, INT) IS
  'Dashboard correlação: purchases/sold_only com limit/offset e totals; ignora merge sintético.';
