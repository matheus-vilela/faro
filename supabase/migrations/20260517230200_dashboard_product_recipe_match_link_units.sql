-- Vínculo ficha ↔ insumo com unidade de consumo e conversão opcional (ex.: 1 UN = 1000 ML, consumo 100 ML).

DROP FUNCTION IF EXISTS public.dashboard_product_recipe_match_link(UUID, UUID, UUID, DOUBLE PRECISION);
...
COMMENT ON FUNCTION public.dashboard_product_recipe_match_link(UUID, UUID, UUID, DOUBLE PRECISION, TEXT, BOOLEAN, TEXT, DOUBLE PRECISION, DOUBLE PRECISION) IS
  'Vincula insumo à ficha com quantidade na unidade escolhida; opcionalmente grava conversão 1 unidade estoque = N unidade secundária.';
