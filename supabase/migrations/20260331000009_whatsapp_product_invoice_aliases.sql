-- Aprendizado: texto da linha da nota (normalizado) → produto escolhido pelo usuário.
-- Usado na importação WhatsApp para sugerir/vincular itens nas próximas notas.

CREATE OR REPLACE FUNCTION public.normalize_invoice_product_label(p_text TEXT)
RETURNS TEXT
LANGUAGE sql
IMMUTABLE
...
CREATE TABLE IF NOT EXISTS public.product_invoice_line_aliases (
...
CREATE INDEX IF NOT EXISTS idx_product_invoice_line_aliases_company
  ON public.product_invoice_line_aliases(company_id);

COMMENT ON TABLE public.product_invoice_line_aliases IS
  'Mapeia texto normalizado da linha da nota (OCR) ao produto cadastrado; reforça sugestões futuras.';

ALTER TABLE public.product_invoice_line_aliases ENABLE ROW LEVEL SECURITY;

-- Catálogo de produtos por token de rascunho (rota pública /w/:token)
CREATE OR REPLACE FUNCTION public.list_products_for_whatsapp_draft(p_token UUID)
RETURNS JSON
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
GRANT EXECUTE ON FUNCTION public.list_products_for_whatsapp_draft(UUID) TO anon, authenticated;

-- finalize: insere expense_items com product_id, cria produto se pedido, grava alias
...
GRANT EXECUTE ON FUNCTION public.normalize_invoice_product_label(TEXT) TO anon, authenticated;
