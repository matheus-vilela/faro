-- Auditoria de unificação em products.merge_audit (JSONB), legado em stock_movements e expense_items,
-- movimentação explícita de merge e RPC de desfazer.

ALTER TABLE public.products
  ADD COLUMN IF NOT EXISTS merge_audit JSONB NOT NULL DEFAULT '[]'::jsonb;

COMMENT ON COLUMN public.products.merge_audit IS
  'Histórico de unificações (JSON array). Fonte da verdade para desfazer merge.';

ALTER TABLE public.expense_items
  ADD COLUMN IF NOT EXISTS metadata_json JSONB NOT NULL DEFAULT '{}'::jsonb;

COMMENT ON COLUMN public.expense_items.metadata_json IS
  'Metadados do item (ex.: product_merge com legado de unificação de produtos).';
...
COMMENT ON FUNCTION public.merge_company_products(UUID, UUID, UUID, NUMERIC, JSONB) IS
  'Unifica produtos com auditoria em merge_audit, legado em movimentações/despesas e movimentação product_merge.';

COMMENT ON FUNCTION public.undo_product_merge(UUID, UUID) IS
  'Desfaz unificação registrada em merge_audit do produto vencedor.';
