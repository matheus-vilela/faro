-- Modo de registro (única / lote) em metadata_json da movimentação manual.
...
CREATE OR REPLACE FUNCTION public.register_manual_stock_movement(
  ...
  p_registration_mode TEXT DEFAULT 'single'
)
RETURNS UUID
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_user_id UUID;
  v_user_name TEXT;
  v_reg_mode TEXT := lower(trim(coalesce(p_registration_mode, 'single')));
BEGIN
  v_user_id := auth.uid();

  IF v_reg_mode NOT IN ('single', 'batch') THEN
    RAISE EXCEPTION 'invalid registration mode: %', p_registration_mode;
  END IF;
...
COMMENT ON FUNCTION public.register_manual_stock_movement(
  UUID, TEXT, TEXT, DECIMAL, DECIMAL, TEXT, DECIMAL, TIMESTAMPTZ, DATE, TEXT
) IS
  'Movimentação manual. p_registration_mode: single | batch.';
