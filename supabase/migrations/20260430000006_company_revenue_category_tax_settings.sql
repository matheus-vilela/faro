-- Taxa de imposto por categoria de receita (folha): configurada em Configurações;
-- create/update de receita ignoram taxa enviada pelo cliente e usam esta tabela (ou 0%).

CREATE TABLE IF NOT EXISTS public.company_revenue_category_tax_settings (
  ...
  CONSTRAINT company_revenue_category_tax_settings_unique UNIQUE (company_id, category_id)
);

CREATE INDEX IF NOT EXISTS idx_company_revenue_cat_tax_company
  ON public.company_revenue_category_tax_settings(company_id);

COMMENT ON TABLE public.company_revenue_category_tax_settings IS
  'Imposto/taxa padrão por folha de receita (DRE); usado ao lançar receitas manuais ou vendas.';

ALTER TABLE public.company_revenue_category_tax_settings ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Company users read revenue category tax settings"
  ON public.company_revenue_category_tax_settings FOR SELECT
  USING (
    company_id IN (SELECT company_id FROM public.user_companies WHERE user_id = auth.uid())
  );

CREATE POLICY "Company users manage revenue category tax settings"
  ON public.company_revenue_category_tax_settings FOR ALL
  USING (
    company_id IN (SELECT company_id FROM public.user_companies WHERE user_id = auth.uid())
  )
  WITH CHECK (
...
GRANT ALL ON public.company_revenue_category_tax_settings TO authenticated;
GRANT ALL ON public.company_revenue_category_tax_settings TO service_role;

-- ---------------------------------------------------------------------------
-- Receitas: taxa definida por folha de receita (nao pelo payload do cliente)
-- ---------------------------------------------------------------------------
...
COMMENT ON FUNCTION public.create_revenue_entry(jsonb) IS
  'Insere receita; taxa/imposto vem de company_revenue_category_tax_settings (ou zero).';
...
COMMENT ON FUNCTION public.update_revenue_entry(jsonb) IS
  'Atualiza receita; taxa/imposto vem de company_revenue_category_tax_settings (ou zero).';
