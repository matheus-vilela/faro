-- Unificação: converte estoque/movimentações do removido para a unidade do vencedor.

DROP FUNCTION IF EXISTS public.merge_company_products(UUID, UUID, UUID);
...
COMMENT ON FUNCTION public.merge_company_products(UUID, UUID, UUID, NUMERIC, JSONB) IS
  'Unifica p_loser em p_winner. p_loser_to_winner_factor: qty_vencedor = qty_removido * fator (obrigatório se unidades de estoque diferem).';

-- Wrapper legado (sem conversão explícita — só válido se unidades iguais)
...
DECLARE
  v_result JSONB;
BEGIN
  v_result := public.merge_company_products(
    p_company_id,
    p_winner_id,
    p_loser_id,
    NULL,
