-- Total do documento na importação e motivo opcional da divergência (valores × itens)

ALTER TABLE public.expenses
  ADD COLUMN IF NOT EXISTS document_total NUMERIC(12, 2),
  ADD COLUMN IF NOT EXISTS divergence_reason TEXT;

COMMENT ON COLUMN public.expenses.document_total IS
  'Total informado no documento (nota) na importação; permite comparar com a soma das linhas no app.';
COMMENT ON COLUMN public.expenses.divergence_reason IS
  'Motivo indicado pelo usuário quando havia divergência ou revisão (ex.: imposto, frete).';
