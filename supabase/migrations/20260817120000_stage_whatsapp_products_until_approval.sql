-- Produtos novos do WhatsApp só entram no catálogo após aprovação da nota.
-- finalize_whatsapp_expense_draft passa a gravar NEW_PRODUCT_STAGED + metadata_json.pending_new_product.
-- approve_whatsapp_expense_as_owner materializa os produtos e então libera o recebimento.
...
BEGIN
  ...
  LOOP
    ...
    IF COALESCE((v_item->>'createProduct')::boolean, false) THEN
      ...
      v_conversions := v_item->'conversions';
      IF v_conversions IS NULL OR jsonb_typeof(v_conversions) IS DISTINCT FROM 'array' THEN
        v_conversions := '[]'::jsonb;
      END IF;

      v_item_product_id := NULL;
      v_import_status := 'NEW_PRODUCT_STAGED';
      v_metadata := jsonb_build_object(
        'pending_new_product', jsonb_strip_nulls(jsonb_build_object(
          'name', v_new_product_name,
          'unit', v_product_unit,
