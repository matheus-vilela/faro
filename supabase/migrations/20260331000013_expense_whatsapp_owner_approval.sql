-- Despesas via WhatsApp ficam pendentes até o proprietário aprovar; só então ganham card de recebimento.

ALTER TABLE public.expenses
  ADD COLUMN IF NOT EXISTS expense_source TEXT NOT NULL DEFAULT 'manual'
  CONSTRAINT expenses_expense_source_check CHECK (expense_source IN ('manual', 'whatsapp'));

COMMENT ON COLUMN public.expenses.expense_source IS
  'manual: fluxo app; whatsapp: importação WhatsApp — aguarda aprovação do owner antes de recebimento válido.';

UPDATE public.expenses
SET expense_source = 'whatsapp'
WHERE notes IS NOT NULL AND notes ILIKE '%Importado via WhatsApp%';

-- Remover cards de recebimento de despesas WhatsApp ainda não aprovadas (passam a existir só após aprovação).
...
BEGIN
  ...
  -- Recebimento só após aprovação do proprietário (RPC approve_whatsapp_expense_as_owner).
...
BEGIN
  ...
  SELECT e.company_id, e.expense_source, e.status
  INTO v_company_id, v_source, v_status
  ...
  IF NOT EXISTS (
    ...
    RETURN json_build_object('success', false, 'error', 'Apenas o proprietário pode aprovar');
  END IF;
